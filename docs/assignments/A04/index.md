...
Design a motor mount attached to a rigid wall for a 24V DC Gear Motor (3.6 kg·cm / 46 RPM w/ 99.5:1 Planetary Gearbox) subjected to a load $P = 300\text{ N}$ at the free end. The design evaluates both yield strength (with a safety factor $N = 3$) and deflection constraints ($\delta_{\text{max}} \le 0.30\text{ mm}$) using beam bending equations.
